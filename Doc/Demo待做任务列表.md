...
> 引擎：UE5　｜　开发方式：纯蓝图　｜　原则：优先复用 UE 内置功能
> 任务粒度：每个任务约 **30分钟** 工作量
...
### A1. 创建项目和初始关卡
- [ ] **A1-1**　创建 UE5 空白项目，选择 BP 模板（First Person 模板可选，但后面需清理不需要的内容）
- [ ] **A1-2**　创建空关卡 `DemoMap`，设为默认地图
- [ ] **A1-3**　搭建 100m×100m 灰盒地面：拖一个 `Plane` 或 `Cube`，Scale 设为 100×1×100，用灰色材质
- [ ] **A1-4**　在四周放置 4 个拉伸方块作为边界围墙（或直接用碰撞体做空气墙）
- [ ] **A1-5**　放置 Ground Plane 的 `NavMeshBoundsVolume` 覆盖整个地面（后续 AI 寻路用）
